...
class AiProvider extends ChangeNotifier {
  ...
  /// Gọi AI tạo ý tưởng
  Future<void> generateIdeasContent(String content, {String? noteId}) async {
    // Validate cơ bản
    if (content.trim().isEmpty) {
      _setErrorMessage('Nội dung cần tạo ý tưởng không được để trống.');
      return;
    }
    ...
    try {
      final request = AiRequest(
        content: content,
        targetLanguage: null, // Không cần thiết cho API ideas
        noteId: noteId,
      );

      final response = await _aiService.generateIdeas(request);
